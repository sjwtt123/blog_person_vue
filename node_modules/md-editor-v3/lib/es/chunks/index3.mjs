import { useId as Fe, provide as T, computed as A, reactive as re, watch as E, onMounted as H, ref as G, inject as w, nextTick as U, shallowRef as ee, onBeforeUnmount as te, toRef as Ae, defineComponent as ce, createVNode as j, Fragment as Re, h as He } from "vue";
...
import { b as y, U as Be, C as Ee, B as X, O as ke, E as Q, P as fe, F as he, f as pe, g as ge, H as ve, h as ye, R as ie, e as je, i as de, d as Ne, j as oe, k as De, T as Ve } from "./event-bus.mjs";
  ...
  const n = w("editorId"), o = w("rootRef"), u = w("usedLanguageText"), c = w("setting"), m = () => {
    o.value.querySelectorAll(`#${n} .${f}-preview .${f}-code`).forEach((i) => {
      ...
      d && (d.onclick = ($) => {
        ...
        const p = (i.querySelector("input:checked + pre code") || i.querySelector("pre code")).textContent, { text: g, successTips: x, failTips: v } = u.value.copyCode;
        let C = x;
  ...
  const d = (h = !1) => {
    ...
    l.forEach((v, C) => {
      ...
      if (h || !v || !v.isConnected || (n?.value ? !n.value.contains(v) : !1)) {
        S?.dispose?.(), b?.disconnect?.();
        return;
      }
      p.push(v), S && g.push(S), b && x.push(b);
    }), l = p, i = g, a = x;
  }, $ = () => {
    d(), !e.noEcharts && c && Array.from(
      ...
      if (p.dataset.closed === "false")
        return !1;
      try {
        const g = new Function(`return ${p.innerText}`)(), x = c.init(p, r.value);
        x.setOption(g), p.setAttribute("data-processed", ""), l.push(p), i.push(x);
...
class rt {
  ...
  deleteExpired(t, r) {
    const n = r ?? this.cache.get(t);
    return !n || !this.isExpired(n) ? !1 : (this.cache.delete(t), !0);
  }
  trim() {
    for (const [t, r] of this.cache)
      if (!this.deleteExpired(t, r))
        break;
  ...
  get(t) {
    ...
    if (!(!r || this.deleteExpired(t, r)))
      return this.cache.delete(t), this.cache.set(t, r), r.value;
  }
  set(t, r) {
...
const Y = new rt({
      ...
      p > 0 && document.body.appendChild(d), await Promise.allSettled(
        Array.from(a).map((g) => (async (v) => {
          ...
          if (!S) {
            ...
            let F = { svg: "" };
            try {
              F = await m.render(b, C, d), S = await e.sanitizeMermaid(F.svg);
              const I = document.createElement("p");
              I.className = `${f}-mermaid`, I.setAttribute("data-processed", ""), I.setAttribute("data-content", C), I.innerHTML = S, I.children[0]?.removeAttribute("height"), Y.set(C, I.innerHTML), v.dataset.line !== void 0 && (I.dataset.line = v.dataset.line), v.replaceWith(I);
            } catch (I) {
              y.emit(t, Q, {
                name: "mermaid",
                message: I.message,
  ...
  e.block.ruler.before(
    ...
    "admonition",
    (i, a, d, $) => {
      let h, p, g, x = !1, v = i.bMarks[a] + i.tShift[a], C = i.eMarks[a];
      if (o !== i.src.charCodeAt(v))
        return !1;
      for (h = v + 1; h <= C && n[(h - v) % u] === i.src[h]; h++)
        ;
      const S = Math.floor((h - v) / u);
      if (S !== r)
        return !1;
      h -= (h - v) % u;
      ...
      for (p = a; p++, !(p >= d || (v = i.bMarks[p] + i.tShift[p], C = i.eMarks[p], v < C && i.sCount[p] < i.blkIndent)); )
        if (o === i.src.charCodeAt(v) && !(i.sCount[p] - i.blkIndent >= 4)) {
          for (h = v + 1; h <= C && n[(h - v) % u] === i.src[h]; h++)
            ;
          if (!(Math.floor((h - v) / u) < S) && (h -= (h - v) % u, h = i.skipSpaces(h), !(h < C))) {
            x = !0;
            break;
          }
        }
      const I = i.parentType, L = i.lineMax;
      return i.parentType = "root", i.lineMax = p, g = i.push("admonition_open", "div", 1), g.markup = b, g.block = !0, g.info = c, g.map = [a, p], m && (g = i.push("admonition_title_open", "p", 1), g.markup = b + " " + c, g.map = [a, p], g = i.push("inline", "", 0), g.content = m, g.map = [a, i.line - 1], g.children = [], g = i.push("admonition_title_close", "p", -1), g.markup = b + " " + c), i.md.block.tokenize(i, a + 1, p), g = i.push("admonition_close", "div", -1), g.markup = i.src.slice(v, h), g.block = !0, i.parentType = I, i.lineMax = L, i.line = p + (x ? 1 : 0), !0;
    },
    {
...
`).length < t.autoFoldThreshold, h = d[1] || t.codeFoldable ? "details" : "div", p = d[1] || t.codeFoldable ? "summary" : "div";
    ...
    if (a[d].hidden)
      return "";
    const g = t.usedLanguageTextRef.value?.copyCode.text, x = t.customIconRef.value.copy || g, v = !!t.customIconRef.value.copy, C = `<span class="${f}-collapse-tips">${Ue("collapse-tips", t.customIconRef.value)}</span>`, [S] = m(a[d]);
    if (S === null) {
      const { open: k, tagContainer: R, tagHeader: J } = l(a[d]), B = [["class", `${f}-code`]];
      k && B.push(["open", ""]);
      const ne = {
        attrs: se(a[d], B)
      };
        ...
        <${R} ${p.renderAttrs(ne)}>
          <${J} class="${f}-code-head">
            ...
            <div class="${f}-code-action">
              ...
              <span class="${f}-copy-button" data-tips="${g}"${v ? " data-is-icon=true" : ""}>${x}</span>
    ...
    for (let k = d; k < a.length && (b = a[k], [F, I] = m(b), F === S); k++) {
      b.info = b.info.replace(o, "").replace(u, ""), b.hidden = !0;
      const R = `${f}-codetab-${t.editorId}-${d}-${k - d}`;
      L = k - d > 0 ? "" : "checked", _ += `
        <li>
          <input
            type="radio"
  ...
  for (const l of u) {
    ...
    for (; g < n; g++) {
      ...
      if (t.src.slice(b, F).trim().endsWith(l.close)) {
        ...
        v = t.src.slice(b, L), x = !0;
        break;
      }
    }
    if (!x)
      continue;
    const S = t.getLines(r + 1, g, t.tShift[r], !0) + (v.trim() ? v : "");
    return s(S, g + 1, l.open);
  }
  return !1;
  ...
  const { editorConfig: r, markdownItConfig: n, markdownItPlugins: o, editorExtensions: u } = P, c = w("editorId"), m = w("language"), s = w(
    "usedLanguageText"
  ), l = w("showCodeRowNumber"), i = w("theme"), a = w("customIcon"), d = w("rootRef"), $ = w("setting"), h = G([]), p = ot(e), g = at(e), { reRenderRef: x, replaceMermaid: v } = lt(e), { reRenderEcharts: C, replaceEcharts: S } = nt(e), b = Ke({
    html: !0,
    breaks: !0,
    linkify: !0
  });
  ...
  b.set({
    highlight: (k, R, J) => {
      ...
      let B;
      !e.noHighlight && p.value ? p.value.getLanguage(R) ? B = p.value.highlight(k, {
        language: R,
        ignoreIllegals: !0
      }).value : B = p.value.highlightAuto(k).value : B = b.utils.escapeHtml(k);
      const ne = l ? Ye(
        B.replace(/^\n+|\n+$/g, ""),
        k.replace(/^\n+|\n+$/g, "")
      ) : `<span class="${f}-code-block">${B.replace(/^\n+|\n+$/g, "")}</span>`;
      return `<pre><code class="language-${R}" language=${R}>${ne}</code></pre>`;
    }
  }), It(b);
...
`)
  ...
  }, K = A(() => (e.noKatex || !!g.value) && (e.noHighlight || !!p.value));
  let Z = -1;
  return E([Ae(e, "modelValue"), K, x, m], () => {
    Z = window.setTimeout(
      () => {
        z();
      },
    ...
    () => {
      $.value.preview && U(() => {
        v().then(q), S(), y.emit(c, oe, h.value);
      });
    }
  ), E([_, C], () => {
    V();
...
`))) : y.emit(r, Ve, a + 1, $);
  ...
  setup(e) {
    ...
    tt(e, u, c), Ft(e, u), At(e, u), Rt(e, u, c);
    const m = A(() => [`${f}-preview`, `${n?.value}-theme`, o && `${f}-scrn`].filter(Boolean)), s = () => {
      const l = `${t}-preview`;
    ...
    return () => j(Re, null, [r.value.preview && (e.previewOnly ? s() : j("div", {
      id: `${t}-preview-wrapper`,
      class: `${f}-preview-wrapper`,
      key: "content-preview-wrapper"
    }, [s()])), r.value.htmlPreview && j("div", {
      id: `${t}-html-wrapper`,
      class: `${f}-preview-wrapper`,
